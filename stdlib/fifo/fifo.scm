...
(define (fifo->string fifo)
  (stringout "#<FIFO "
    ...
    "-" (fifo-running fifo) "-" (fifo-waiting fifo) " "
...
(defrecord (fifo MUTABLE OPAQUE `(stringfn . fifo->string))
  ...
  (waiting 0) ;; How many threads are waiting on the FIFO.
  (running 0) ;; How many threads are processing results from the FIFO.
...
(define (fifo/waiting fifo (nonzero #t))
  "Returns the number of thread waiting on the fifo"
  (unwind-protect
      (begin (condvar-lock (fifo-condvar fifo))
	(when nonzero
...
(define (fifo/paused? fifo)
  (and (fifo-pause fifo) (zero? (fifo-running fifo))))
